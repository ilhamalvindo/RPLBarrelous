...
from schemas import BahanDasar, BebanSchema, PengeluaranSchema, PembayaranSchema, SupplyBahanDasar, BebanSchema, UserSchema
...
async def login(user : UserSchema, db: Session=Depends(get_db)) :
    ...
    return (user.username == userCheck.username and bcrypt.verify(userCheck.password, user.password))
